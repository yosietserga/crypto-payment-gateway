...
// No need for local enum, using imported AuditLogAction
 ...
 */
const verifyBinanceWebhookSignature = (req: Request, res: Response, next: NextFunction) => {
  try {
    const payload = JSON.stringify(req.body);
    const signature = req.headers['binance-signature'];
    const timestamp = req.headers['binance-timestamp'];
    ...
    try {
      ...
      if (!transaction) {
        ...
        await auditLogRepository.save({
          ...
          entityId: 'UNMATCHED',
          action: AuditLogAction.BINANCE_DEPOSIT,
          data: {
            txId,
            amount,
            coin,
